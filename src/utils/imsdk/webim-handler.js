import webim from './webim-wx.js'

const sessions = {};
let loginSuccessListeners = [];
var selToID
  ,loginInfo
  ,accountMode
  ,accountType
...
function sdkLogin(userInfo, listeners, options) {
  ...
  webim.login(userInfo, listeners, options,
    function (identifierNick) {
      ...
      if (loginSuccessListeners.length) {
        loginSuccessListeners.forEach((cb) => {
          cb();
        });
...
// 发送消息(普通消息)
function onSendMsg({ content, toId, random, msgTime, onSuccess }) {
  if (!sessions[toId]) {
    sessions[toId] = new webim.Session(selType, toId, toId, null, Math.round(new Date().getTime() / 1000));
  }
